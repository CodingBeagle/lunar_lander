...
pub fn load_obj(file_path: PathBuf) -> ObjLoaderResult {
    ...
    for current_line_result in lines {
        ...
        if parts[0] == "v" {
            vertices.push(parts[1].parse::<f32>().expect("failed to convert vertice string to number"));
            vertices.push(parts[2].parse::<f32>().expect("failed to convert vertice string to number"));
            vertices.push(parts[3].parse::<f32>().expect("failed to convert vertice string to number"));
        }
